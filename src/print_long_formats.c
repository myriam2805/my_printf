...
** my_printf
** File description:
** Functions to print long integers with flags
*/
#include "../include/my.h"

static int print_long_no_sign(unsigned long nb)
{
    int count = 0;

    if (nb >= 10)
        count = count + print_long_no_sign(nb / 10);
...
static void print_long_output(unsigned long nb, int is_negative,
    const flags_t *flags, int precision_zeros)
{
    print_int_sign(is_negative, flags);
    print_zeros(precision_zeros);
    print_long_no_sign(nb);
}

static void print_long_padding(const flags_t *flags, int padding)
{
...
static int calc_long_total_len(int num_len, int precision_zeros,
    int is_negative,
    const flags_t *flags)
{
...
{
    int is_negative = 0;
    unsigned long unsigned_nb;
    int num_len;
    int precision_zeros;
    int total_len;
    int padding;

    unsigned_nb = (nb < 0) ? (unsigned long)(-nb) : (unsigned long)nb;
    if (nb < 0)
        is_negative = 1;
    num_len = count_ulong_digits(unsigned_nb);
    precision_zeros = calc_precision_zeros(flags, num_len);
    total_len = calc_long_total_len(num_len, precision_zeros,
        is_negative, flags);
    padding = (flags->width > total_len) ? flags->width - total_len : 0;
    print_long_padding(flags, padding);
    print_long_output(unsigned_nb, is_negative, flags, precision_zeros);
    if (flags->minus == 1)
        print_spaces(padding);
